...
LLM-driven agent, a deterministic Risk Engine + evidence-checked Report Writer, a
...
phase is done, what a report should say. Everything else — validating that decision,
executing it, persisting evidence, scoring risk, checking a report's claims against
real evidence — is deterministic Python. The LLM never runs arbitrary commands and
never builds tool arguments outside a fixed schema.
...
- **Evidence-checked reports** — the LLM drafts; every domain/IP/CVE/port/count it
  claims is checked against that scan's own stored evidence and dropped or
...
- **Benchmark lab** — synthetic ground-truth scenarios that run the *real* agent
  (never a real domain) and score its precision/recall — this is how a genuine
...
lookups can execute. Full policy, the exact first-run consent text, and the complete
guardrail-to-code mapping: **[docs/ethics.md](docs/ethics.md)**.
...
- [eval/README.md](eval/README.md) — the benchmark lab: how to run it, what it measures
